...
import { hospitalReadingText, hospitalReadingTranslation, hospitalQuestions, hospitalVocab } from "@/data/hospitalReadingData";
...
export default function HospitalReadingPractice({ onBack, lang }: Props) {
  ...
  const questions = useMemo(() => shuffleArray(hospitalQuestions).map(q => ({ ...q, options: shuffleArray(q.options) })), []);
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="text-2xl font-bold text-primary mb-4">🏥 {t("בבית החולים", "At the Hospital")} <span className="text-sm bg-green-100 text-green-800 px-2 py-1 rounded">{t("קל", "Easy")}</span></h1>
          
          <Card className="mb-4">
            <CardContent className="p-6">
              <p className="text-lg leading-relaxed whitespace-pre-wrap" style={{ lineHeight: 1.8 }}>{hospitalReadingText}</p>
              ...
              {showTranslation && <p className="mt-3 text-muted-foreground whitespace-pre-wrap" dir="ltr">{hospitalReadingTranslation}</p>}
            </CardContent>
          </Card>
