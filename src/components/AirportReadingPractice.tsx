...
import { airportReadingText, airportReadingTranslation, airportQuestions, airportVocab } from "@/data/airportReadingData";
import { shuffleArray } from "@/lib/shuffleArray";

interface Props { onBack: () => void; lang: "he" | "en"; }

export default function AirportReadingPractice({ onBack, lang }: Props) {
  ...
  const questions = useMemo(() => shuffleArray(airportQuestions).map(q => ({ ...q, options: shuffleArray(q.options) })), []);
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="text-2xl font-bold text-primary mb-4">✈️ {t("בשדה התעופה", "At the Airport")} <span className="text-sm bg-yellow-100 text-yellow-800 px-2 py-1 rounded">{t("בינוני", "Medium")}</span></h1>
          
          <Card className="mb-4">
            <CardContent className="p-6">
              <p className="text-lg leading-relaxed whitespace-pre-wrap" style={{ lineHeight: 1.8 }}>{airportReadingText}</p>
              ...
              {showTranslation && <p className="mt-3 text-muted-foreground whitespace-pre-wrap" dir="ltr">{airportReadingTranslation}</p>}
            </CardContent>
          </Card>
