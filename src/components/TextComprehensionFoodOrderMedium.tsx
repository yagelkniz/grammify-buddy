...
import { readingText, translationMap } from "./FoodOrderMediumReadingData";
  ...
  return (
    <div className="flex flex-col items-center justify-center p-7 gap-7 bg-orange-50 rounded-2xl shadow-lg border-[3px] border-orange-300 max-w-2xl mx-auto mt-8 rtl transition-all">
      ...
      <div className="flex flex-col gap-3 w-full">
        {questions.map((q, i) => (
          <div key={i} className="w-full bg-white border border-orange-200 rounded-xl p-3 shadow transition-all hover:shadow-lg">
            <div className="mb-2 font-semibold text-md flex items-center gap-2" dir="rtl">
              <span className="text-orange-400">{i + 1}.</span> {q.question}
            </div>
              ...
              <div className="mt-1 flex items-center gap-2">
                {feedbacks[i] === "correct" ? (
                  ...
                  <span className="text-red-700 font-bold flex items-center gap-1">
                    ❌ טעות{questions[i].joke && <> – <span className="text-xs">{questions[i].joke}</span></>}
                  </span>
                )}
              </div>
            )}
      ...
      <div className="mt-5 flex flex-col items-center gap-2 w-full">
        <div className="font-bold text-lg text-orange-900 bg-orange-100 px-5 py-2 rounded shadow" dir="rtl">
          סטטיסטיקה: {correct} תשובות נכונות מתוך {questions.length}
        </div>
        {finished && (
          <div className="text-2xl font-bold text-orange-700 mt-1" dir="rtl">
