...
import { cityVocab, cityQuestions } from "@/data/cityVocabData";
import { shuffleArray } from "@/lib/shuffleArray";

interface Props { onBack: () => void; lang: "he" | "en"; }

export default function CityVocabPractice({ onBack, lang }: Props) {
  const [phase, setPhase] = useState<"learn" | "quiz">("learn");
  const [current, setCurrent] = useState(0);
  const [selected, setSelected] = useState<string | null>(null);
  const [score, setScore] = useState(0);
  const t = (he: string, en: string) => lang === "he" ? he : en;
  const questions = useMemo(() => shuffleArray(cityQuestions).map(q => ({ ...q, options: shuffleArray(q.options) })), []);
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="text-2xl font-bold text-primary mb-6">🏙️ {t("בעיר — אוצר מילים", "In the City — Vocabulary")}</h1>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            {cityVocab.map(v => (
              <Card key={v.hebrew}>
                <CardContent className="p-4">
                  <div className="flex justify-between items-center">
                    <span className="text-lg font-bold">{v.hebrew}</span>
                    <span className="text-sm bg-secondary px-2 py-1 rounded">{v.english}</span>
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">{v.example}</p>
                  <p className="text-xs text-muted-foreground" dir="ltr">{v.exampleEn}</p>
                </CardContent>
              </Card>
            ))}
          </div>
