...
import { familyMembers, familyQuestions } from "@/data/familyData";
import { shuffleArray } from "@/lib/shuffleArray";

interface Props { onBack: () => void; lang: "he" | "en"; }

export default function FamilyPractice({ onBack, lang }: Props) {
  const [phase, setPhase] = useState<"learn" | "quiz">("learn");
  const [current, setCurrent] = useState(0);
  const [selected, setSelected] = useState<string | null>(null);
  const [score, setScore] = useState(0);
  const t = (he: string, en: string) => lang === "he" ? he : en;
  const questions = useMemo(() => shuffleArray(familyQuestions).map(q => ({ ...q, options: shuffleArray(q.options) })), []);
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="text-2xl font-bold text-primary mb-6">👨‍👩‍👧 {t("משפחה — אוצר מילים", "Family — Vocabulary")}</h1>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
            {familyMembers.map(m => (
              <div key={m.hebrew} className={`bg-card rounded-lg p-3 border shadow-sm ${m.gender === "male" ? "border-l-4 border-l-blue-400" : m.gender === "female" ? "border-l-4 border-l-pink-400" : ""}`}>
                <div className="font-bold text-lg">{m.hebrew}</div>
