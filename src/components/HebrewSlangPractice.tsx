...
import { hebrewSlangLevels, SlangLevel, SlangQuestion } from "@/data/hebrewSlangData";
...
export default function HebrewSlangPractice({ onBack, lang = "he" }: Props) {
  ...
  return (
    <LevelQuiz
      level={hebrewSlangLevels[activeLevelIndex]}
      lang={lang}
      t={t}
      onFinish={(score) => {
        setLevelScores((prev) => ({
          ...prev,
          [hebrewSlangLevels[activeLevelIndex].levelId]: score,
        }));
    ...
    <div dir="rtl" className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <h1 className="text-2xl font-bold text-primary text-center">
          {t("🗣️ סלנג ישראלי", "🗣️ Israeli Hebrew Slang")}
        </h1>
        <p className="text-center text-muted-foreground text-sm">
          {t(
            "תרגלו ביטויי סלנג ישראליים מהחיים האמיתיים: סבבה, יאללה, חבל על הזמן...",
            "Practice real-life Israeli slang expressions: sababa, yalla, chaval al hazman..."
          )}
        </p>
...
function LevelQuiz({
  ...
  const options = lang === "he" ? current.optionsHe : current.optionsEn;
  ...
  const prompt = lang === "he" ? current.promptHe : current.promptEn;
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm text-muted-foreground text-center">
              {lang === "he" ? level.instructionsHe : level.instructionsEn}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            {/* Prompt */}
            <div className="text-lg font-semibold text-center leading-relaxed p-4 bg-muted/50 rounded-lg whitespace-pre-line" dir="rtl">
              {prompt}
            </div>
            ...
            <div className="grid grid-cols-1 gap-3">
              {shuffledIndices.map((optIdx) => {
                ...
                let extraClass = "text-base py-4 font-medium text-right justify-start";
              ...
              <div className="space-y-3">
                <div className={cn(
                  ...
                  {isCorrect
                    ...
                    : `${t("לא נכון. התשובה:", "Incorrect. The answer:")} ${options[current.correctIndex]}`}
                </div>
